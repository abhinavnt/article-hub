...
export const authMiddleware = (roles: UserRole[]) => {
  return (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!accessToken) res.status(401).json({ error: "Unauthorized: No token provided" });

      const decoded = jwt.verify(accessToken, process.env.JWT_SECRET!) as { userId: string; role: string };
