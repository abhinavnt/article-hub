...
export const Dashboard: React.FC = () => {
  ...
  const loadArticles = async () => {
    ...
    try {
      ...
      if (showAllCategories) {
        fetchedArticles = await articleService.getAllArticles()
      } else {
        fetchedArticles = await articleService.getArticlesByPreferences(user.articlePreferences)
      }
  ...
  const handleDislike = async (id: string) => {
    setActionLoading(id)
    try {
      await articleService.dislikeArticle(id)
      setArticles((prev) =>
        prev.map((article) =>
          article.id === id
            ? {
  ...
  const handleBlock = async (id: string) => {
    setActionLoading(id)
    try {
      await articleService.blockArticle(id)
